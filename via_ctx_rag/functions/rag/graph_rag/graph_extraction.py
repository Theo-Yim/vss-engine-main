...
from via_ctx_rag.utils.ctx_rag_batcher import Batch
...
class GraphExtraction:
    def __init__(self, batcher, llm, graph: Neo4jGraphDB):
        self.graph_db = graph
    ...
    async def update_embedding_chunks(self, chunkId_chunkDoc_list, file_name):
        with TimeMeasure(
    ...
    def create_relation_between_chunks(self, file_name) -> list:
    ...
    async def create_entity_embedding(self):
        ...
        for i in range(0, len(rows), 1000):
            await self.update_embeddings(rows[i : i + 1000])
    ...
    async def apost_process(self):
        with TimeMeasure("GraphRAG/aprocess-doc/graph-create/postprocessing", "green"):
            ...
            chunkId_chunkDoc_list = self.create_relation_between_chunks(file_name)
            await self.update_embedding_chunks(chunkId_chunkDoc_list, file_name)
