...
from utils import MediaFileInfo, get_available_gpus, get_avg_time_per_chunk
...
class ListFilesResponse(ViaBaseModel):
    ...
    data: list[AddFileInfoResponse] = Field(max_length=100)
...
class AlertTool(ViaBaseModel):
    ...
    events: list[Annotated[str, Field(max_length=1024, pattern=DESCRIPTION_PATTERN)]] = Field(
...
class ViaServer:
    ...
    def _setup_routes(self):
        ...
        async def add_video_file(
            ...
            # Cleaning up older files; to allow Q&A to work on newer videos
            # Needed only for BP Preview
            # TODO: Remove this code
            if "FORCE_CA_RAG_RESET" in os.environ and bool(os.environ["FORCE_CA_RAG_RESET"]):
                logger.info("Resetting CA RAG before adding new file")
                if self._stream_handler:
                    self._stream_handler.reset_ca_rag()
            if file:
                ...
                if videoId in self._stream_handler._live_stream_info_map:
                    ...
                    try:
                        request_id = await loop.run_in_executor(
                            ...
                            query.summarize,
                            query.enable_chat,
                            False,  # query.enable_cv_metadata,
