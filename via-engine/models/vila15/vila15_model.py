        ...
        if use_trt:
            ...
            self._output_tpool = (
                concurrent.futures.ThreadPoolExecutor(
                    max_workers=max_batch_size * 2 if self.TRTLLM_EXECUTOR_INFLIGHT_BATCHING else 2
                )
                if async_output
                else None
            )
            ...
            # Pytorch Model
            with TimeMeasure("VILA model load"):
                # Load PyTorch model
                device_map = "cuda"
...
if __name__ == "__main__":
    vila = Vila15("/opt/models/llava-v1.5-sharegpt-ptft-13b-lita-soft-ce-im-vid-se", True)
